...
--##! Apps
hl.bind("SUPER + Return", hl.dsp.exec_cmd(terminal), { description = "Aplicación: Terminal" })
--hl.bind("SUPER + T", hl.dsp.exec_cmd(terminal))
hl.bind("CTRL + ALT + T", hl.dsp.exec_cmd(terminal))
hl.bind("SUPER + E", hl.dsp.exec_cmd(fileManager), { description = "Aplicación: Gestor de archivos" })
hl.bind("SUPER + W", hl.dsp.exec_cmd(browser), { description = "Aplicación: Navegador" })
hl.bind("SUPER + C", hl.dsp.exec_cmd(codeEditor), { description = "Aplicación: Editor de código" })
hl.bind("CTRL + SUPER + SHIFT + ALT + W", hl.dsp.exec_cmd(officeSoftware), { description = "Aplicación: Ofimática" })
hl.bind("SUPER + X", hl.dsp.exec_cmd(textEditor), { description = "Aplicación: Editor de texto" })
hl.bind("CTRL + SUPER + V", hl.dsp.exec_cmd(volumeMixer), { description = "Aplicación: Mezclador de volumen" })
hl.bind("SUPER + I", hl.dsp.exec_cmd(settingsApp), { description = "Aplicación: Configuración" })
hl.bind("CTRL + SHIFT + Escape", hl.dsp.exec_cmd(taskManager), { description = "Aplicación: Administrador de tareas" })
hl.bind("XF86Calculator", hl.dsp.exec_cmd("kcalc"), { description = "Aplicación: Calculadora" })

--##! PWAs (Aplicaciones Web)
hl.bind("SUPER + SHIFT + A", hl.dsp.exec_cmd("brave --app=https://gemini.google.com"), { description = "PWA: Gemini" })
hl.bind("SUPER + SHIFT + K", hl.dsp.exec_cmd("brave --app=https://claude.ai/new"), { description = "PWA: Claude" })

hl.bind("SUPER + SHIFT + W", hl.dsp.exec_cmd("brave --app=https://web.whatsapp.com"), { description = "PWA: WhatsApp" })
hl.bind("SUPER + SHIFT + N", hl.dsp.exec_cmd("brave --app=https://app.notion.com"), { description = "PWA: Notion" })
...
for i = 1, 10 do
    ...
    end, { description = "Escritorio: Ir a escritorio " .. i })
end
...
for i = 1, 2 do
    ...
    local prefix = { "r-", "r+" }
    local descdir = { "izquierdo", "derecho" }
    hl.bind("CTRL + SUPER + " .. keys[i], hl.dsp.focus({ workspace = prefix[i] .. "1" }), {description = "Escritorio: Ir a escritorio " .. descdir[i]})
end
for i = 1, 2 do
    local keys = { "Left", "Right" }
    local prefix = { "m-", "m+" }
...
--## Special
hl.bind("SUPER + S", hl.dsp.workspace.toggle_special("special"), { description = "Escritorio: Mostrar oculto (Scratchpad)" })
...
--##! Session
hl.bind("SUPER + L", hl.dsp.exec_cmd("loginctl lock-session"), { description = "Sesión: Bloquear pantalla" })
hl.bind("SUPER + SHIFT + L", hl.dsp.exec_cmd("systemctl suspend || loginctl suspend"),
    { locked = true, description = "Sesión: Suspender" }) -- Sleep
...
hl.bind("CTRL + SHIFT + ALT + SUPER + Delete", hl.dsp.exec_cmd("systemctl poweroff || loginctl poweroff"),
    { description = "Sesión: Apagar" }) -- # [hidden] Power off

--##! Uncategorized
--hl.bind("SUPER + space", hl.dsp.exec_cmd("~/.config/hypr/launcher.sh"), { description = "Aplicación: Lanzador" })
hl.bind("SUPER + space", hl.dsp.exec_cmd("fuzzel"), { description = "Aplicación: Lanzador" })
--hl.bind("SUPER + Tab", hl.dsp.global("quickshell:overviewWorkspacesToggle"), { description = "Sistema: Vista general de escritorios" })
hl.bind("SUPER + V", hl.dsp.global("quickshell:overviewClipboardToggle"))
hl.bind("SUPER + Period", hl.dsp.global("quickshell:overviewEmojiToggle"))
hl.bind("SUPER + Minus", hl.dsp.global("quickshell:cheatsheetToggle"), { description = "Sistema: Mostrar atajos (Cheatsheet)" })
hl.bind("SUPER + M", hl.dsp.global("quickshell:mediaControlsToggle"), { description = "Sistema: Controles multimedia" })
--hl.bind("SUPER + G", hl.dsp.global("quickshell:overlayToggle"), { description = "Sistema: Alternar widgets" })
hl.bind("CTRL + ALT + Delete", hl.dsp.global("quickshell:sessionToggle"), { description = "Sistema: Menú de apagado/sesión" })
--hl.bind("SUPER + J", hl.dsp.global("quickshell:barToggle"), { description = "Sistema: Mostrar/ocultar barra" })
...
--  { description = "Sistema: Cambiar fondo de pantalla" })
--hl.bind("CTRL + SUPER + ALT + T", hl.dsp.global("quickshell:wallpaperSelectorRandom"),
--  { description = "Sistema: Fondo de pantalla aleatorio" })
hl.bind("CTRL + SUPER + SHIFT + D", hl.dsp.global("quickshell:toggleLightDark"),
    { description = "Sistema: Alternar modo claro/oscuro" })
hl.bind("CTRL + SUPER + R", hl.dsp.exec_cmd("killall ydotool qs quickshell; qs -c $qsConfig &"),
    { description = "Sistema: Reiniciar widgets" })
hl.bind("CTRL + SUPER + P", hl.dsp.global("quickshell:panelFamilyCycle"), { description = "Sistema: Cambiar estilo de panel" })
--# Testing
...
hl.bind("SUPER + V", hl.dsp.exec_cmd(
    ...
    { description = "Utilidad: Historial del portapapeles" })
hl.bind("SUPER + Period", hl.dsp.exec_cmd(
        qsIsAlive .. " || pkill fuzzel || " .. hyprScripts .. "/fuzzel-emoji.sh copy"),
    { description = "Utilidad: Selector de Emojis" })
hl.bind("SUPER + SHIFT + S", hl.dsp.global("quickshell:regionScreenshot"), { description = "Utilidad: Recorte de pantalla" })
hl.bind("SUPER + SHIFT + S",
    hl.dsp.exec_cmd(qsIsAlive .. " || pidof slurp || hyprshot --freeze --clipboard-only --mode region --silent"))
hl.bind("SUPER + SHIFT + G", hl.dsp.global("quickshell:regionSearch"), { description = "Utilidad: Buscar en Google Lens" })
...
hl.bind("SUPER + SHIFT + X", hl.dsp.global("quickshell:regionOcr"),
    { description = "Utilidad: Extraer texto de imagen (OCR)" })
--hl.bind("SUPER + SHIFT + T", hl.dsp.global("quickshell:screenTranslate"),
--  { description = "Utilidad: Traducir pantalla" })
...
hl.bind("SUPER + SHIFT + C", hl.dsp.exec_cmd("hyprpicker -a"),
    { description = "Utilidad: Selector de color (Pipeta)" })
--# Recording stuff
hl.bind("SUPER + SHIFT + R", hl.dsp.global("quickshell:regionRecord"),
    { locked = true, description = "Utilidad: Grabar región (sin audio)" })
...
hl.bind("SUPER + SHIFT + ALT + R", hl.dsp.exec_cmd(qsScripts .. "/videos/record.sh --fullscreen --sound"),
    { locked = true, description = "Utilidad: Grabar pantalla (con audio)" })
...
), { locked = true, non_consuming = true, description = "Utilidad: Captura de pantalla completa" })
...
hl.bind("SUPER + SHIFT + ALT + mouse:273", hl.dsp.exec_cmd(hyprScripts .. "/ai/primary-buffer-query.sh"),
    { description = "Utilidad: Resumen IA del texto seleccionado" })
...
hl.bind("SUPER + Minus", function() zoomfunction(-0.3) end, { repeating = true, description = "Pantalla: Alejar (Zoom out)" })
hl.bind("SUPER + Equal", function() zoomfunction(0.3) end, { repeating = true, description = "Pantalla: Acercar (Zoom in)" })
...
--##! Window
--# Focusing
hl.bind("SUPER + J", hl.dsp.layout("togglesplit"), { description = "Ventana: Cambiar orientación de división" })
hl.bind("SUPER + mouse:272", hl.dsp.window.drag(), { mouse = true, description = "Ventana: Mover" })
--hl.bind("SUPER + mouse:274", hl.dsp.window.drag(), { mouse = true })
hl.bind("SUPER + mouse:273", hl.dsp.window.resize(), { mouse = true, description = "Ventana: Redimensionar" })
--#/# bind = SUPER + ←/↑/→/↓,, -- Focus in direction
for i = 1, 4 do
    local arrowkey = { "Left", "Right", "Up", "Down" }
    local focusdir = { "l", "r", "u", "d" }
    hl.bind("SUPER + " .. arrowkey[i], hl.dsp.focus({ direction = focusdir[i] }),
        { description = "Ventana: Enfocar hacia " .. arrowkey[i] })
end
for i = 1, 2 do
...
for i = 1, 4 do
    ...
    hl.bind("SUPER + SHIFT + " .. arrowkey[i], hl.dsp.window.move({ direction = focusdir[i] }),
        { description = "Ventana: Mover hacia " .. arrowkey[i] })
end
...
hl.bind("ALT + F4",
    ...
    { non_consuming = true })
hl.bind("SUPER + Q", hl.dsp.window.close(), { description = "Ventana: Cerrar" })
hl.bind("SUPER + SHIFT + ALT + Q", hl.dsp.exec_cmd("hyprctl kill"), { description = "Ventana: Forzar cierre (Kill)" })
...
--# Positioning mode
hl.bind("SUPER + T", hl.dsp.window.float({ action = "toggle" }), { description = "Ventana: Alternar Flotante/Mosaico" })
hl.bind("SUPER + D", hl.dsp.window.fullscreen({ mode = "maximized", action = "toggle" }),
    { description = "Ventana: Maximizar" })
hl.bind("SUPER + F", hl.dsp.window.fullscreen({ mode = "fullscreen", action = "toggle" }),
    { description = "Ventana: Pantalla completa" })
hl.bind("SUPER + ALT + F", hl.dsp.window.fullscreen_state({ internal = 0, client = 3, action = "toggle" }),
    { description = "Ventana: Falsa pantalla completa" })
hl.bind("SUPER + P", hl.dsp.window.pin(), { description = "Ventana: Fijar (Pin)" })
...
for i = 1, 10 do
    ...
    end, { description = "Ventana: Enviar a escritorio " .. i })
end
...
for i = 1, 2 do
    ...
    local prefix = { "r-", "r+" }
    local descdir = { "izquierda", "derecha" }
    hl.bind("SUPER + SHIFT + Page_" .. keydirs[i], hl.dsp.window.move({ workspace = prefix[i] .. "1" }), {description = "Ventana: Enviar a escritorio " .. descdir[i]})
end
for i = 1, 4 do
...
hl.bind("SUPER + ALT + S",
    hl.dsp.window.move({ workspace = "special:special", follow = false }), { description = "Ventana: Enviar a escritorio oculto (Scratchpad)" })
